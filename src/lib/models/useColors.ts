export const useColors = () => {
  const generateBackgroundColors = (baseColor: string): Record<number, string> => {
    ...
    colors[0] = baseColor; // Just in case someone wants to use the base color directly

    return colors;
  };

  const generateColors = (baseColor: string, overlayColor: string, opacity: number): Record<string, string> => {
    ...
    for (const colorStep of colorSteps) {
      const baseRgb = hexToRgb(backgroundColors[Number(colorStep)]);
      ...
      colors[Number(colorStep)] = resultingColor;
    }

    colors[0] = overlayColor; // Just in case someone wants to use the base color directly
