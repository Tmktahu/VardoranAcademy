...
{#if data.book.isAvailable === false && data.book.requiresPassword === true}
  <PasswordModal 
  ...
  />
{:else if data.book.isAvailable === false}
  <div class="flex flex-col items-center justify-center min-h-screen text-center p-8">
    ...
    <button class="mt-6 bg-background-700 hover:bg-background-600 px-6 py-2 rounded text-white" on:click={() => goto(resolve(`/librarian/library/${data.category}`))}>
      Return to Library
    </button>
  </div>
{:else}
